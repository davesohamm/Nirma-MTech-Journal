...
**Click here to read:** [May Journal](https://github.com/davesohamm/Nirma-MTech-Journal/blob/main/May/README.md)

---

# 1 June 2026 (Monday) - Day 234

## Morning

Reached the office around **10:00 AM**. Had **peanut butter toast** and **Chocos milk** for breakfast before heading to my workstation.

Today, we were officially assigned to our respective projects. I was assigned to the **QA project – AI Infra Agent**. After joining the project's communication group, I introduced myself and asked for the next steps. The first half of the day was mostly spent getting updates from the team, understanding the project structure, and waiting for further instructions.

## Afternoon

After lunch, we were instructed to shift our seating area to the **Broadband Team** section. We moved our workstations accordingly and settled into the new workspace.

Later, I met **Rajat Bhai** and **Kunal Kumar**, who will be leading and guiding me on the project. They provided an overview of the project and explained the onboarding process. I was granted access to **Cursor Enterprise**, which will be used for development activities throughout the internship.

The next step was to obtain access to the required internal tools such as **GitLab**, **Jenkins**, and other project resources. I raised the necessary **JIRA requests** and waited for approval.

While waiting, I spent some time learning and exploring concepts related to:
...
This helped me gain a basic understanding of the technologies likely to be used in the project.

## Evening

From around **4:00 PM to 5:15 PM**, I had a video call with *her* while enjoying my daily milk break.

After that, I attended a quick meeting session regarding **Freshers' Policies**, where important organizational guidelines and expectations were discussed.

In the evening, we had snacks consisting of **Dhokla** and **Frankie** before wrapping up for the day.

I left the office and reached home around **7:30 PM**.

## Night

After freshening up and completing the usual daily chores, I had a video call with my family and caught up with everyone.

Later, I spoke with *her* again over a call and spent some quality time talking.

Before ending the day, I solved **one LeetCode problem**, maintaining my coding practice streak.

Overall, the day was primarily focused on project onboarding, access setup, team introductions, and beginning my learning journey into AI agents and related technologies.
...
# 30 June 2026 (Tuesday) - Day 255

## Morning

Reached the office around **11:20 AM** today. We got delayed because of heavy traffic, so after reaching the office, I had breakfast in a hurry and headed straight to my workstation.

The day began with the regular Scrum discussion with **Rajat Bhaiya** and **Kumar Kunal**, where we reviewed the current project status, pending tasks, and the priorities for the day.

Our primary focus was improving the **Grafana Dashboard** that Rajat Bhaiya had developed.

Together, we enhanced the dashboard by adding more meaningful observability metrics. Initially, we configured it to display:

- LLM calls within a selected time frame
- Context window size
- Number of characters processed during each request

After validating those metrics, we further refined the dashboard to present information **per user session**, making the monitoring much more useful for debugging and performance analysis.

The updated dashboard now provides much better visibility into the behavior of the AI pipeline and allows us to analyze model usage at a much finer granularity.

Once everything was verified, we successfully merged the updated dashboard into the **master branch**.
...
After lunch, we shifted our focus toward testing and improving the project's deployment consistency.

During testing, we observed that the project was producing **different results on different machines**.

After investigating the issue, we discovered that the root cause was inconsistent environment variable configurations across development environments.

To solve this, we redesigned the configuration structure.

Instead of storing every configuration inside environment variables, we:

- Moved all configurable application settings into a centralized **YAML configuration file**
- Kept only sensitive secrets and API keys inside the `.env` file
- Updated the application to read configuration from the new structure

This change made the project significantly more portable and ensured consistent behavior across different development environments.

After implementing the new configuration approach, we updated the **Runbook** accordingly so that future developers could easily set up the project using the new configuration process.

Once everything was configured, we verified that the project now produced **consistent results across multiple devices**.

## Additional Improvements

Later in the afternoon, I fixed a frontend issue in the automation workflow where the **Cursor CLI** status was incorrectly displayed as **Failed** even when it was actually running successfully.

After debugging the frontend logic, I updated the implementation so that the status now correctly reflects the real execution state.

This small fix improves the user experience by providing accurate pipeline status information.

## Documentation & Knowledge Sharing

Another major part of today's work involved improving the project's internal documentation.

We created a new **Confluence Wiki space** under the **QA AI Agent** section.

I wrote my **first internal blog**, documenting how to:

- Set up the project locally
- Configure the environment
- Execute the Runbook
- Get the application running successfully

In addition to the setup guide, I also created a **homepage** for the repository that provides a centralized overview of:
...
Finally, I went through all the existing project documentation and updated it to reflect the numerous architectural and implementation changes that Rajat Bhaiya and I had introduced over the past several weeks.

It felt satisfying to not only improve the codebase but also leave behind clear documentation that will help future developers onboard more easily.

## Architecture Decision

One interesting conclusion from today's experimentation was related to the **Headroom Framework**.

After comparing its compaction algorithms with our own proprietary implementation, we found that **our custom compaction algorithms consistently performed better** for our specific use case.

As a result, we decided **not to integrate the Headroom framework** into the project and instead continue using our own optimized implementation.

It was reassuring to see that the architecture we had designed over the past few weeks proved to be the better solution.

## Evening

After **5:30 PM**, we headed to the cafeteria for evening snacks.
...
After work, I reached home at around **6:50 PM**.
...
I also talked with *her*, and together we finished the movie that we had started the previous day. It was a relaxing and peaceful way to end the evening after a productive day at work.

Looking back, today was a satisfying blend of engineering, debugging, documentation, and architectural decision-making. Improving observability through Grafana, standardizing project configuration, fixing deployment inconsistencies, contributing to the team's knowledge base with my first internal blog, and confirming the effectiveness of our own compaction algorithms all felt like meaningful contributions.

June has been one of the most transformative months of my internship. Every day has brought new challenges, deeper technical understanding, and opportunities to contribute in ways I hadn't imagined at the beginning of the journey.

**Day Ended.**

---

## 📌 End of June Journals
